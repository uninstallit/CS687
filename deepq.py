...
def create_q_model(num_inputs, num_hidden, num_actions):
    ...
    common = tf.keras.layers.Dense(num_hidden, activation="relu")(common)
    action = tf.keras.layers.Dense(num_actions, activation="linear")(common)
...
def main():
    ...
    # make a action.
    model = create_q_model(num_inputs, num_hidden, num_actions)
    # Build a target model for the prediction of future rewards.
    # The weights of a target model get updated every 10000 steps thus when the
    # loss between the Q-values is calculated the target Q-value is stable.
    model_target = create_q_model(num_inputs, num_hidden, num_actions)
    ...
    while True:
        ...
        for timestep in range(1, max_steps_per_episode):
            ...
            state_next, reward, done, _ = pong.step(action, timestep)
